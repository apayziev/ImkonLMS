...
def upgrade() -> None:
    ...
    # --- school_settings ---
    if not inspector.has_table("school_settings"):
        op.create_table(
            "school_settings",
        ...
        # Table exists but may be missing timetable columns
        _add_column_if_not_exists("school_settings", sa.Column("day_start_time", sa.String(5), nullable=False, server_default="08:00"))
        _add_column_if_not_exists("school_settings", sa.Column("day_end_time", sa.String(5), nullable=False, server_default="16:00"))
        _add_column_if_not_exists("school_settings", sa.Column("lesson_duration_minutes", sa.SmallInteger(), nullable=False, server_default="45"))
        _add_column_if_not_exists("school_settings", sa.Column("default_break_minutes", sa.SmallInteger(), nullable=False, server_default="5"))
        _add_column_if_not_exists("school_settings", sa.Column("working_days", sa.ARRAY(sa.SmallInteger()), nullable=False, server_default="{1,2,3,4,5,6}"))
        _add_column_if_not_exists("school_settings", sa.Column("breaks", sa.JSON(), nullable=False, server_default="[]"))

    # --- time_slot ---
    if not inspector.has_table("time_slot"):
        op.create_table(
            "time_slot",
            sa.Column("id", sa.Integer(), autoincrement=True, nullable=False),
    ...
    # --- schedule_entry ---
    if not inspector.has_table("schedule_entry"):
        op.create_table(
            "schedule_entry",
            sa.Column("id", sa.Integer(), autoincrement=True, nullable=False),
            sa.Column("academic_year_id", sa.Integer(), sa.ForeignKey("academic_year.id"), nullable=False),
